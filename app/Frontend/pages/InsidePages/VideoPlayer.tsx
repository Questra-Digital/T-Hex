...
const VideoPlayer = ({ data }: any) => {
  ...
  const [id_c, setid_C] = useState(0); // Changed initial state to 0
  const [testfile, settestfile] = useState(data[0].testfile);
  const [duration, setduration] = useState(data[0].duration);
  const [videourl, setvideourl] = useState(`/video.mp4`); // Updated to use relative path
  ...
  const handleNextPage = () => {
    ...
    // Assuming all videos are named v1.mp4, you might want to update this logic based on your data
    setvideourl(`/v1.mp4`);
  };
    ...
    <>
      ...
      <div className="flex justify-center">
        <div className="mb-3 inline-flex mt-2 xs:mt-0">
          <button className="text-sm text-indigo-50 transition duration-150 hover:bg-indigo-500 bg-indigo-600 font-semibold py-2 px-4 rounded-l" onClick={handlePreviousPage}>
            Prev
          </button>
          &nbsp; &nbsp;
          <button className="text-sm text-indigo-50 transition duration-150 hover:bg-indigo-500 bg-indigo-600 font-semibold py-2 px-4 rounded-r" onClick={handleNextPage}>
            Next
          </button>
        </div>
      </div>
