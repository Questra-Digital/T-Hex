...
// func StartTest(ctx context.Context, input model.StartTestInput) (string, error) {
...
// 		TestPath: input.ProjectPath,
// 	}

// 	if err := db.Table("tests").Create(&tests).Error; err != nil {
// 		return "Test Table Failed", err
// 	}
// 	return "Created Test Table", nil
// }
...
func DeleteAllTests(ctx context.Context) error {
	...
	/* Waleed data must come from projectInfo Table*/

	username := lastProj.GitEmail              /*"abdullahsaleem40404@gmail.com"*/
	filename := lastProj.GitTestCaseFileName   /*"python"*/
	nooftestcases := lastProj.GitNoOfTestCases /*2*/
	language := lastProj.GitLanguage           /*"Python"*/
	...
	/*Stop and Remove fluentd containers*/
...
func InitiateTest(ctx context.Context, ID int, GitEmail string, GitProjectName string, GitLanguage string, GitNoOfTestCases int, GitTestCaseFileName string) (string, error) {
	...
	username := GitEmail              /*"abdullahsaleem40404@gmail.com"*/
	projectname := GitProjectName     /*"FYP-Sample-Python-Test"*/
	filename := GitTestCaseFileName   /*"python"*/
	nooftestcases := GitNoOfTestCases /*2*/
	language := GitLanguage           /*"Python"*/
	...
	// 		return "Error creating dockerfile", err
	// 	}
	// }
	...
	for i := 1; i < nooftestcases+1; i++ {
		...
		// Retrieve the IP address from the inspect response
	...
	for i := 1; i < nooftestcases+1; i++ {
		...
		// Construct the Docker command to wait for the container
		...
		// Extract the exit status code from the output
	...
	// username string, projectname string, nooftestcases int, language string, duration string
...
func functionToPushToDrive(username string, projectname string, nooftestcases int, testfile string, duration []string, test_result []string) (string, error) {
		...
		for _, file := range files {
			...
			if filepath.HasPrefix(fileName, filePrefix) {
				if strings.HasSuffix(fileName, ".log") {
					...
					// fluentArray = append(fluentArray, file.id)
					if err != nil {
						return "Error", err
					}
				}
	...
	// for index, fluentdelement := range fluentArray {
	// 	fmt.Printf("Element is located at %d and value %s\n", index, fluentdelement)
	// }

	// var ScreenShots []appendData
	...
	for i := 0; i < nooftestcases; i++ {
		...
		for _, file := range files {
			...
			if strings.HasSuffix(fileName, ".png") {
				...
				// fmt.Println(fileName)
				// ScreenShots = append(ScreenShots, appendData{testID: i + 1, name: fileName})
			}
...
func CreateDockerFile(ctx context.Context, input model.CreateDockerFileInput) (string, error) {
	...
	username := lastProj.GitEmail              /*"abdullahsaleem40404@gmail.com"*/
	projectname := lastProj.GitProjectName     /*"FYP-Sample-Python-Test"*/
	filename := lastProj.GitTestCaseFileName   /*"python"*/
	nooftestcases := lastProj.GitNoOfTestCases /*2*/
	language := lastProj.GitLanguage           /*"Python"*/
