...
func RunContainerForProjectTestImageForParallel(client *client.Client, imagename string, containername string, port string, inputEnv []string, containerip string, i string, baseDir string, basePath string, wg *sync.WaitGroup) {
	defer wg.Done()
	...
	if err != nil {
		log.Printf("Error starting container %s: %s\n", containername, err)
		return
	}

	log.Printf("Container %s is created and started", cont.ID)
}
...
func RunContainerConcurrently(client *client.Client, containers []ContainerInfo) {
	...
	for _, container := range containers {
		wg.Add(1)
		go RunContainerForProjectTestImageForParallel(client, container.ImageName, container.ContainerName, container.Port, container.InputEnv, container.ContainerIP, container.ContainerIndex, container.BaseDir, container.BasePath, &wg)
	}
